...
namespace Komsky.Web.Controllers
{
    ...
    public partial class AccountController : Controller
    {
        ...
        // Registration is not allowed. Only Admins can create users
        //[AllowAnonymous]
        ...
        #region Basic authorization before accessing site
        [AllowAnonymous]
        public virtual ActionResult Access(string returnUrl)
        {
            ViewBag.ReturnUrl = returnUrl;
            return View();
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public virtual ActionResult Access(AccessViewModel model, string returnUrl)
        {
            if (!ModelState.IsValid)
            {
                return View(model);
            ...
            bool isAuthorized = model.UserName == "Komsky" && model.Password == "Pa$$w0rd";

            if (isAuthorized)
            {
                string encodedValue = string.Format("{0}{1}", model.UserName, model.Password).ToBase64Encode();
                string cookieName = BasicAuthorizationCookie.GetCookieName(Request.Url);
                Response.Cookies.Set(new HttpCookie(cookieName, encodedValue)); // Session cookie
